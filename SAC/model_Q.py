...
class GaussianPolicy(nn.Module):
    def __init__(self, num_inputs, num_actions, hidden_dim, nf_fac,action_space=None,init_zeros = False):
        super(GaussianPolicy, self).__init__()

        features = int(num_inputs*(num_inputs + 1)/2*(nf_fac)**2)
        ...
        self.q3 = nn.Linear(features, num_actions)
        self.q4 = nn.Linear(features, num_actions)
        
        self.apply(weights_init_)
        
        self.q1 = nn.Linear(num_inputs, features, bias = False)
        self.q2 = nn.Linear(num_inputs, features, bias = False)
        if init_zeros:
            self.q1.weight.data.fill_(0.00)
            self.q2.weight.data.fill_(0.00)
    ...
    def forward(self, state):
        ...
        b = F.relu(self.linear1(state))
        b = F.relu(self.linear2(b))
        mean =  self.q3(a) + self.mean_linear(b)
        log_std = self.q4(a) + self.log_std_linear(b)
...
class DeterministicPolicy(nn.Module):
    ...
    def forward(self, state):
        ...
        b = F.relu(self.linear1(state))
        b = F.relu(self.linear2(b))
        mean = torch.tanh(self.q3(a) + self.mean(b)) * self.action_scale + self.action_bias
        return mean
